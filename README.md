...
cp -r devops/vps-backup-recovery ~/.hermes/skills/devops/vps-backup-recovery/

# Or install directly from SKILL.md URL (if your runtime supports it)
<agent-cli> skills install https://raw.githubusercontent.com/iizcm/vps-backup-recovery-skill/main/SKILL.md
```
...
```text
skill_view(name="devops/vps-backup-recovery")
```
...
```text
Use the vps-backup-recovery skill to do its primary task.
```
